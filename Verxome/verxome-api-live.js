...
const VerxomeAPI = (() => {
  ...
  const CONFIG = {
    ...
    PAYSTACK: {
      PUBLIC_KEY: 'pk_live_e538030407c0e304925b16f6fc8ce641fe39dc47',
      CURRENCY: 'GHS',
    },
  };

  // ---------------------------------------------------------------------
  // Launch promo — 50% off every plan until the end date below.
  //
  // This is the ONLY place the discount is defined. Prices shown on the
  // page, the amount actually charged through Paystack, and the ticker
  // banner all read from this same isPromoActive()/applyPromoPricing()
  // logic — so nothing needs to be manually turned off after the promo
  // ends. The moment a visitor's (or the server's) clock passes END_DATE,
  // everything automatically reverts to full price on its own.
  //
  // IMPORTANT: worker.js has its OWN copy of this same END_DATE, used to
  // independently verify the charged amount server-side. If you ever
  // change the date or discount here, update it there too, or genuine
  // payments could get rejected (or the wrong amount could get accepted).
  // ---------------------------------------------------------------------
  const PROMO = {
    ACTIVE: true, // master on/off switch, independent of the date check below
    DISCOUNT_PERCENT: 50,
  ...
  function applyPromoPricing(plan) {
    const active = isPromoActive();
    const originalPrice = plan.price;
    const discountedPrice = Math.round(originalPrice * (1 - PROMO.DISCOUNT_PERCENT / 100));
    return {
      ...plan,
      price: active ? discountedPrice : originalPrice, // the effective price — used for display AND the actual Paystack charge
      originalPrice: originalPrice,
      discountActive: active,
      discountBadge: active ? PROMO.BADGE_TEXT : null,
    };
  }
  ...
  function sendAdminNotificationEmail(application, plan) {
    ...
    const templateParams = {
      ...
      membership_end_date: application.membershipEndDate,
      message:
        ...
        `Registration Date: ${application.registrationDate}\n` +
        `Membership End Date: ${application.membershipEndDate}`,
    };
  ...
  async function getPlans() {
    if (CONFIG.MODE === 'live') {
      ...
      return plans.map(applyPromoPricing);
    }
    return Object.values(PLANS).map(applyPromoPricing);
  }
  ...
  function getPlan(planId) {
    ...
    return plan ? applyPromoPricing(plan) : null;
  }
  ...
  async function submitApplication(formData) {
    ...
    const membershipEndDate = new Date(registrationDate);
